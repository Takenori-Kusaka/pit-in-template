...
ピットイン方式（標準）では、人間と AI の役割分担および委譲してはならない領域を厳格に定義しています。

### 1. 標準が定める「一律で AI に委譲できない領域」（第5章 5.8.3）
次の領域は、AIの自律レベルにかかわらず、人間からの委譲が**一律に禁止**されています。

- **安全に関する最終的な判断**（第5章 5.8.3 項目1）
- **人間のレビューを経ない安全関連コードの変更**（第5章 5.8.3 項目2）
- **形式的な検証活動（テストや静的解析など）そのものの代替**（第5章 5.8.3 項目3 / 例: テストを動かさないまま通過とする等）
- **挙動の非決定性（AIの気まぐれやエラーなど）を隠す目的の実装**（第5章 5.8.3 項目4）
- **認証・認可・個人データ・外部との契約に関わる決定**（第3章 3.4.1）
- **適用範囲（標準の適用境界）そのものの拡大**（第3章 3.4.1）

### 2. 標準ロール定義に基づく担当割り当て（第5章 5.5）
次の4つの基本判断は、標準第5章 5.5における「人間の担当ロール（価値責任者、技術判断者など）」の規定に基づき、**テンプレートの初期値として人間が担うこと**を強く推奨（デフォルト制約）しています。
*注意: これらは判断の担当を割り当てる指針であり、標準本文が一律でAIへの委譲を恒久的に禁止している規定ではありません（将来の能力向上や体制に応じて役割境界を見直すことが可能です [第5章 5.9]）。*
...
*※ テンプレートの初期設定（CLAUDE.md/D-0等）では安全側に倒して「人間が担う」ものとしていますが、案件の特性、体制、AI能力の見直しなどに基づき、これらをAIサブエージェントへ委譲する緩和・テーラリングを行う場合は、その設計判断と根拠を必ず **ADR（判断記録）** へ記録してください。*
...
しかし、ソフトウェア工学における実測データ（2025–2026年）によると、AIによる一括コード生成はPRサイズを50%〜150%増加させ、それに伴い人間のレビュー待ち時間が平均440%以上増大する**「シニアエンジニア税（レビューのボトルネック化）」**や、重大な欠陥率が40%上昇する**「サイレントな品質低下（見落とし）」**を引き起こすことが確認されています。

したがって、プロジェクトや組織の判断により**あえて大きなPRサイズを許容する運用（大バッチ運用）を採用する場合**は、人間の認知限界を補うために、標準が定める以下の**「代償措置（自動検証の強化）」**を同時に実装・運用しなければなりません。
